...
export class CoreComponentStack extends Component
{
    static appcoreClass = "app.js.component-stack";
    ...
    #transitionMode = CoreComponentStack.TransitionMode.SLIDE_X + 
                      CoreComponentStack.TransitionMode.SCALE +                      
                      CoreComponentStack.TransitionMode.FADE;
    ...
    {
        super.onLoad();
        await Loader.loadStyle("app/styles/component-stack.css");
    ...
    _getComponentTransform(index, activeIndex)
    {
        ...
        if (this.#transitionMode == CoreComponentStack.TransitionMode.DEFAULT)
        {
            return "none";
        }
        ...
        if (this.#transitionMode.includes(CoreComponentStack.TransitionMode.SLIDE_X))
        {
            transform += ` translateX(${index * 100}%)`;
        }

        if (this.#transitionMode.includes(CoreComponentStack.TransitionMode.SLIDE_Y))
        {
            transform += ` translateY(${index * 100}%)`;
        }

        if (this.#transitionMode.includes(CoreComponentStack.TransitionMode.SCALE))
        {
            transform += ` scale(${active ? "1" : this.#transitionMinScale})`;
        }

        if (this.#transitionMode.includes(CoreComponentStack.TransitionMode.ROTATE))
        {
            if (active)
            {
        ...
        if (this.#transitionMode.includes(CoreComponentStack.TransitionMode.FLIP_X))
        {
            if (active)
            {
        ...
        if (this.#transitionMode.includes(CoreComponentStack.TransitionMode.FLIP_Y))
        {
            if (active)
            {
    ...
    set active(componentId)
    {
        ...
        for (const [, component] of this.childs)
        {
            ...
            component.node.style.opacity = this.#transitionMode.includes(CoreComponentStack.TransitionMode.FADE) ? (active ? "1" : "0") : "1";
